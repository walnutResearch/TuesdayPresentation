#!/usr/bin/env python3
"""
Density estimation model: multi-scale CNN for patch-wise walnut density (count) prediction. Uses 6-channel
input (RGB + grayscale + edge). Train on synthetic or real data; MPS/Apple Silicon friendly. Defines
MultiScaleDensityNet, ModelConfig, MultiChannelTransform; can be run as script to train.

How to run (training):
  python density_model.py --synthetic_data path/to/synthetic_data [--output path/to/models] [--epochs 50]
